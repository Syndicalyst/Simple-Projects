...
SpellCaster::SpellCaster(const std::string& name, int x, int y,
    ...
    debugPrint("SpellCaster created", this->getName());
}
...
SpellCaster::~SpellCaster() {
    ...
    debugPrint("SpellCaster destroyed", this->getName());
}
...
void SpellCaster::castAction(Unit& unit, Spell& spell) {
    if ( !isAlive() ) {
        std::cout << "Unit " << this->getName() << " is dead and can't cast" << std::endl;
        return;
    }
    
    if ( !this->canCast ) {
        std::cout << "\033[35m" << "Unit " << this->getName() << " can't cast anymore" << "\033[30m" << std::endl;
        return;
    }
    
    if ( !haveMana() ) {
        std::cout << "Unit " << this->getName() << " don't have mana" << std::endl;
        return;
    }
    
    if ( !haveEnoughMana(spell) ) {
        std::cout << "Unit " << this->getName() << " don't have enough mane for " <<spell.getName() << " spell" << std::endl;
        return;
    }
    ...
    if ( this->distance(unit) > SPELL_DIST ) {
        std::cout << "\033[37m" << unit.getName() << " " << unit.getType() << " is too far for " << this->getName() << "'s " << this->getType() << " cast" << "\033[30m" << std::endl;
        return;
    }
