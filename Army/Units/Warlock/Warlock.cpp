...
Warlock::Warlock(const std::string& name, int x, int y, int mana, int hp, int dmg)
    ...
    debugPrint("Warlock created", this->getName());
}
...
Warlock::~Warlock() {
    ...
    debugPrint("Warlock destroyed", this->getName());
}
...
void Warlock::castAction(Spell& spell) {
    ...
    if ( !this->haveEnoughMana(spell) ) {
        std::cout << "Unit " << this->getName() << " don't have enough mana for " <<spell.getName() << " spell" << std::endl;
    }
...
void Warlock::showSpecial() const {
    ...
    if ( this->demons.empty() ) {
        std::cout << this->getName() << " don't have slaves" << "\033[30m" << std::endl;
    } else {
